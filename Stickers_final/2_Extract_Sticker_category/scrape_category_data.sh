...
[ -d all_stickers ] && echo "Directory already exists" || mkdir all_stickers
category_list=("mostpopular" "expressions" "tvseries" "characters" "summer" "movies" "animatedstickers" "sports" "food" "bbmfamily" "stickerfactory" "animalsandpets" "favindonesia" "loveandromance" "emoji" "workandoffice" "classics" "games")
...
echo "$k"
	...
	echo "$j"
		...
		do
		fileone=$j
	    	file_two=categorystickers
    		new_file_name=$fileone$file_two$x$k
	        echo "$new_file_name"
     		part_one='https://fiji.bbmessaging.com/api/v1/sticker_packs?platform=android&country='
		part_two='&cap=GIF_STK&category='
    		part_three='&start='
		part_four='&limit=2100'
	    	api=$part_one$j$part_two$k$part_three$x$part_four
    		echo "$api" 
	    	curl -s "$api" | jq '.stickerpacks[] |  [.id,.name,.animated,.licenseType] |@csv ' -M >  all_stickers/"$new_file_name"
    		x=$((x+120))
		done
	done
done
